...
class PC:
    ...
    def dfs(self, graph, k: int, chain: List[int], visit: List[bool]):
        """因果关系链深搜"""
        flag = False
    ...
    def get_causal_chains(self, graph, start: int, labels: List[str]):
        global chains
        chains = []
        ...
        self.dfs(graph, start, chain, visit)

        return "\n".join([" <- ".join(list(map(lambda x: labels[x] + f" ({x})", c))) for c in chains])

    def plot(self, graph, labels: List[str], path: str = Optional[None]):
        """可视化学习出的贝叶斯网络"""
        G = nx.DiGraph()  # 创建空有向图
    ...
    def skeleton(self, suff_stat, alpha: float):
        ...
        while done != True and any(G):
            ...
            for x, y in pairs:
                if G[x][y] == True:
                    neighbors = self.get_neighbors(G, x, y)  # adj(C,x) \ {y}
    ...
    def pc(self, suff_stat, alpha: float = 0.05, verbose: bool = False):
        G, O = self.skeleton(suff_stat, alpha)  # 骨架
        cpdag = self.extend_cpdag(G, O)  # 扩展为 CPDAG
...
if __name__ == "__main__":
    file_path = "./Generate_Data/Causal/Data/test.csv"
    image_path = "./Generate_Data/Causal/Graph/causal_graph.png"
    ...
    # DFS 因果关系链
    print(pc_.get_causal_chains(p, start=2, labels=labels))

    # 画图
    pc_.plot(p, labels, image_path)