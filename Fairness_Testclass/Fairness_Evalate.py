...
class Fairness_Evalute:
    ...
    def separation(self,x_tranin,sensitive_param, y_target, y_predict):
        ...
        FN_un = data_un.loc[(data_un['target'] == target_lvls[0]) & (data_un['predicted'] == target_lvls[1])].shape[0]
        FP_un = data_un.loc[(data_un['target'] == target_lvls[1]) & (data_un['predicted'] == target_lvls[0])].shape[0]
        TP_un = data_un.loc[(data_un['target'] == target_lvls[0]) & (data_un['predicted'] == target_lvls[0])].shape[0]
        TN_un = data_un.loc[(data_un['target'] == target_lvls[1]) & (data_un['predicted'] == target_lvls[1])].shape[0]
        FPR_un = FP_un / (TN_un + FP_un)
        TPR_un = TP_un / (TP_un + FN_un)

        #保护属性
        data_priv = data[data['sens'] == sens_lvls[1]]
        FN_priv = data_priv.loc[(data_priv['target'] == target_lvls[0]) & (data_priv['predicted'] == target_lvls[1])].shape[0]
        FP_priv = data_priv.loc[(data_priv['target'] == target_lvls[1]) & (data_priv['predicted'] == target_lvls[0])].shape[0]
        TP_priv = data_priv.loc[(data_priv['target'] == target_lvls[0]) & (data_priv['predicted'] == target_lvls[0])].shape[0]
        TN_priv = data_priv.loc[(data_priv['target'] == target_lvls[1]) & (data_priv['predicted'] == target_lvls[1])].shape[0]
