...
class ADF:
    def __init__(self, perturbation_size) -> None:
...
#The  implementation of local perturbation
    class Local_Perturbation:
        ...
        def __init__(self, sess, grad, x, n_value, sens, input_shape, conf, perturbation_size):
            """
        ...
        def __call__(self, x):
            ...
            x = ADF.clip(x + s * local_cal_grad, self.conf).astype("int")

            return x

#The implementation of ADF
    def dnn_fair_testing(self, dataset, sensitive_param, model_path, cluster_num, max_global, max_local, max_iter):
        """
        The implementation of ADF
        ...
        # select the seed input for fairness testing，excute global perturbation
        inputs = self.seed_test_input(clusters, min(max_global, len(X)))
        ...
        for num in range(len(inputs)):
            ...
            for iter in range(max_iter+1):
                ...
                # search the instance with maximum probability difference for global perturbation， including local generation
                ...
                if label != n_label and (tuple(temp) not in global_disc_inputs) and (tuple(temp) not in local_disc_inputs):
                    ...
                    #num代表的是现在用的是原始数据的第num个种子，它是针对每个种子先全局然后局部，然后再选下一个种子，并不是每一个原始种子都能走到局部阶段，因为在全局上一直找不到歧视性输入
                    #suc_idx是当前生成的所有全局歧视性输入
                    # start local perturbation
                    minimizer = {"method": "L-BFGS-B"}
                    local_perturbation = self.Local_Perturbation(sess, grad_0, x, n_value, sensitive_param, input_shape[1],
                                                            data_config[dataset],self.perturbation_size)
                ...
                sample[0] = ADF.clip(sample[0] + self.perturbation_size * cal_grad[0], data_config[dataset]).astype("int")
        ...
        np.save('./Generate_Data/results/'+dataset+'/'+ str(sensitive_param) + '/suc_idx.npy', np.array(suc_idx))
        np.save('./Generate_Data/results/'+dataset+'/'+ str(sensitive_param) + '/global_samples.npy', np.array(global_disc_inputs_list))
        np.save('./Generate_Data/results/'+dataset+'/'+ str(sensitive_param) + '/local_samples.npy', np.array(local_disc_inputs_list))
        np.save('./Generate_Data/results/'+dataset+'/'+ str(sensitive_param) + '/disc_value.npy', np.array(value_list))
...
def main(argv=None):
    FLAGS = flags.FLAGS
    perturbation_size = 1
    adf = ADF(perturbation_size)
    adf.dnn_fair_testing(dataset = FLAGS.dataset,
